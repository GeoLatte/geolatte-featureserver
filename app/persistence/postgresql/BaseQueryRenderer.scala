...
trait BaseQueryRenderer extends QueryRenderer[String, RenderContext] {
  ...
  def renderBooleanAnd(
    lhs: BooleanExpr,
    rhs: BooleanExpr)(implicit ctxt: RenderContext): String = s" ( ${render(lhs)} ) AND ( ${render(rhs)} )"

  def renderBooleanOr(
    lhs: BooleanExpr,
    rhs: BooleanExpr)(implicit ctxt: RenderContext): String = s" ( ${render(lhs)} ) OR ( ${render(rhs)} )"
  ...
  def renderBetween(
    lhs: AtomicExpr,
    lb: AtomicExpr,
    up: AtomicExpr): String = s" ( ${renderAtomic(lhs)} between ${renderAtomic(lb)} and ${renderAtomic(up)} ) "

  def renderComparisonPredicate(
    lhs: AtomicExpr,
    op: ComparisonOperator,
    rhs: AtomicExpr)(implicit ctxt: RenderContext): String = s" ${renderAtomicCasting(lhs, rhs)} ${sym(op)} ( ${renderAtomic(rhs)} )"

  def renderInPredicate(
    lhs: AtomicExpr,
    rhs: ValueListExpr)(implicit ctxt: RenderContext): String = s" ${renderAtomicCasting(lhs, rhs)} in ${renderValueList(rhs)}"

  def renderRegexPredicate(
    lhs: AtomicExpr,
    rhs: RegexExpr)(implicit ctxt: RenderContext): String = s" ${renderAtomicCasting(lhs, rhs)} ~ '${rhs.pattern}'"

  def renderLikePredicate(
    lhs: AtomicExpr,
    rhs: LikeExpr,
    caseSensitive: Boolean = true)(implicit ctxt: RenderContext): String = if (caseSensitive) s" ${renderAtomicCasting(lhs, rhs)} like '${rhs.pattern}'"
  else s" ${renderAtomicCasting(lhs, rhs)} ilike '${rhs.pattern}'"

  def renderNullTestPredicate(
    lhs: AtomicExpr,
    is: Boolean)(implicit ctxt: RenderContext): String = s" ${renderAtomic(lhs)} ${if (is) "is" else "is not"} null"

  def renderToDate(
    date: AtomicExpr,
    fmt: AtomicExpr): String = s" to_date(${renderAtomic(date)}, ${renderAtomic(fmt)}) "
  ...
  def renderJsonContains(
    lhs: PropertyExpr,
    rhs: LiteralString)(implicit ctxt: RenderContext): String = s"${renderPropertyExpr(lhs)}::jsonb @> '${rhs.value}'::jsonb "
