  ...
  "A QueryParser " should {
    ...
    "Interpret literal numbers  properly" in {
      ...
      (
        ...
        QueryParser.parse( "var =  123.54" ) must beSuccessfulTry[BooleanExpr].withValue(
        ...
        QueryParser.parse( " var =  123.54E-3" ) must beSuccessfulTry[BooleanExpr].withValue(
    ...
    "supports the in operator" in {
      (QueryParser.parse( "var in ('a', 'b', 'c')" ) must beSuccessfulTry[BooleanExpr].withValue(
        InPredicate( PropertyExpr( "var" ), ValueListExpr( List( LiteralString( "c" ),
                                                                 LiteralString( "b" ),
                                                                 LiteralString( "a" )
        )
        )
